...
function MediumCard({
  ...
  return (
    <Link
      href={href}
      className={cn(
        "group relative block aspect-[3/2] min-h-0 w-full overflow-hidden rounded-lg border border-border/50 shadow-xs ring-1 ring-black/[0.04] transition",
...
export function HomeExploreByMediumSection({
  ...
  return (
    <HomePastelSection variant={variant}>
      ...
      />
      <ul className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 md:grid-cols-3">
        {mediumItems.map((m) => {
